...
class PdoDriver implements DriverInterface
{
     ...
     * @param array $bind
     *            Values to bind to the prepared statement.
    ...
    final public function simpleFetchOne(string $sql, array $bind = []): ?array
    {
        ...
        if (count($retVal) > 1) {
            throw new UnexpectedResultException('SIMPLE_FETCH_ONE_RETURNED_MORE_THAN_ONE_RESULT', [
                'sql' => $sql,
                'bind' => $bind,
                'return' => $retVal
            ]);
    ...
    final public function simpleFetch(string $sql, array $bind = []): array
    {
        try {
            ...
            throw new QueryException('SIMPLE_QUERY_EXCEPTION', [
                'sql' => $sql,
                'bind' => $bind,
                'exception' => $e
            ], $e);
    ...
    final public function simpleExecute(string $sql, array $bind = []): array
    {
        try {
            ...
            throw new QueryException('SIMPLE_EXECUTE_EXCEPTION', [
                'sql' => $sql,
                'bind' => $bind,
                'exception' => $e
            ], $e);
     ...
     *            An identifier to disambiguate between multiple concurrent queries
     * @return PdoDriver
     * @throws QueryException If the query resulted in an Exception.
     */
    final public function prepare(string $sql, string $identifier = 'single'): PdoDriver
    {
        try {
            ...
            throw new QueryException('PREPARE_EXCEPTION', [
                'sql' => $sql,
                'exception' => $e
            ], $e);
        }
     ...
     *
     * @param array $bind
     *            Array of binds for the prepared statement.
     * @param string $identifier
     *            An identifier to disambiguate between multiple concurrent queries
     * @return array
     * @throws QueryException If the query resulted in an Exception.
    ...
    final public function execute(array $bind, string $identifier = 'single'): array
    {
        try {
            ...
            throw new QueryException('EXECUTE_EXCEPTION', [
                'bind' => $bind,
                'exception' => $e
            ], $e);
        }
     ...
     *            Orders the results by this field name
     * @param string $sortDirection
     *            ASC or DESC
     * @param int $limit
     *            Limits the number of results
     * @param int $page
     *            Returns the $page'th page of results limited by $limit
    ...
    /**
     * Prepares a bind array.
     * Converts bind array parameters into CSV strings.
     *
     * @param string $sql
     * @param array $bind
     * @return array
    ...
    final public function prepareBind(string $sql, array $bind): array
    {
        foreach ($bind as $identifier => $bindItem) {
            if (is_array($bindItem)) {
                if (empty($bindItem)) {
                    throw new QueryException('BIND_PARAM_EMPTY', [
                        'sql' => $sql,
                        'bind' => $bind
                    ]);
                }
